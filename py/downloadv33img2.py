...
# 小图映射JSON路径
SMALL_IMG_JSON = "E:\\www\\test\\py\\json\\imgSmallMapping.json"
...
def process_image(image_path, output_path):
    ...
    try:
        with Image.open(image_path) as img:
            ...
            print(f"─=≡Σ(((つ•̀ω•́)つ=== 裁剪完成: {os.path.basename(image_path)} (比例 {CROP_RATIO_BOTTOM})")
...
# 保存图片（带重试机制）
# -------------------
def save_image_with_retry(url, save_path, crop_path):
    """带重试机制的图片下载函数"""
    for attempt in range(RETRY_COUNT):
        try:
            ...
            # 如果文件已经存在，直接跳过
            if os.path.exists(save_path) and os.path.exists(crop_path):
                print(f"⚠️ 图片已存在，跳过: {save_path}")
                return True
            ...
            if response.status_code == 200:
                ...
                # 下载完成后进行裁剪
                process_image(save_path, crop_path)
                print(f"✅ 成功下载并处理: {save_path}")
                return True
            else:
...
def generate_save_directory(data_type, data_key):
    ...
    data_key_dir = os.path.join(subdir, data_key)
    # 原始图片目录
    original_dir = os.path.join(data_key_dir, "original")
    # 裁剪后图片目录
    crop_dir = os.path.join(data_key_dir, "cropped")

    os.makedirs(original_dir, exist_ok=True)
    os.makedirs(crop_dir, exist_ok=True)

    return original_dir, crop_dir
    ...
    try:
        while True:
            ...
            try:
                ...
                # 创建保存目录
                original_dir, crop_dir = generate_save_directory(data_type, data_key)
                ...
                for img_url in img_urls:
                    ...
                    save_path = os.path.join(original_dir, img_name)
                    crop_path = os.path.join(crop_dir, img_name)

                    # 下载图片（带重试）
                    if not save_image_with_retry(img_url, save_path, crop_path):
                        all_success = False
