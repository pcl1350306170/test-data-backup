# 写一个脚本，处理A目录下的所有txt文件，转为epub ，并自动添加封面
* 1、获取“D:\book\HH”【A目录】下面的所有txt文件，【A目录】下面的所有txt文件，合并的时候，以每个txt文件的文件名作为目录名，比如存在“张三医嘱信息.txt”“张三护理记录.txt”，生成的epub文件章节需要是“第一章 张三医嘱信息”“第二章 张三护理记录”。同时删除txt文件内部原有的章节，确保生成的epub文件章节是txt文件的文件名。
* 2、封面目录是"D:\book\封面"
* 3、epub输出目录是"D:\book\epub-py"，没有这个目录就给我建立
* 4、生成的EPUB 文件要求：
* * 遵守 XHTML 1.1 标准格式。
* * 保证 <meta> 与 <link> 标签的写法符合规范。
* * 输出文件强制使用 UTF-8 编码，无 BOM。
* 5、保留“epub_image_in.py”里面合并换行的功能和插入图片的功能，获取"D:\book\封面"所有图片，已经控制少于100张。
* 6、拓展功能：读取“C:\www\test\py\json\novelMapping.json”里面的数据，是一个对象，格式是：
...
```
* 需要获取所有文本，把 "肝ai""ganai"替换为"肝癌"，替换novelMapping.json里面的所有配置项
...
该脚本实现了将指定目录下的所有TXT文件合并转换为单个EPUB电子书，并满足以下需求：

1. **章节处理**：
    - 将"A目录"（D:\book\HH）下的每个TXT文件作为一个章节
    - 章节标题格式为"第X章 文件名"（例如"第一章 张三医嘱信息"）
    - 自动移除TXT文件内部原有章节结构，仅保留内容

2. **封面与输出**：
    - 从"D:\book\封面"目录自动获取封面图片
    - 输出目录为"D:\book\epub-py"，不存在时自动创建

3. **格式规范**：
    - 生成符合XHTML 1.1标准的内容
    - 严格遵守标签规范，使用正确的XML命名空间
    - 强制使用UTF-8编码（无BOM）

4. **保留功能**：
    - 保留`epub_image_in.py`中的段落合并功能（合并未以中文标点结尾的段落）
    - 保留随机插入图片功能（最多使用100张图片）

5. **文本替换**：
    - 读取"novelMapping.json"中的替换规则
    - 对所有文本内容进行批量替换（如将"肝ai"和"ganai"替换为"肝癌"）

运行脚本后，会在输出目录生成一个包含所有TXT内容的EPUB文件，文件名以源目录名（HH）命名，包含自动生成的目录和封面。处理过程会记录在日志文件中，方便追踪转换情况。

# 我验证了一下这个脚本，文本里面的内容并没有按照“C:\www\test\py\json\novelMapping.json”里面的规则替换字符，给我修改一下，其他功能不用改。

问题出在`replace_text`函数的实现上，当前代码只是简单使用字符串替换，但可能存在编码或替换逻辑的问题。以下是修复后的代码，仅修改了文本替换相关部分：
...
1. 改进了`load_replacement_rules`函数：
   - 增加了文件存在性检查
   - 使用`utf-8-sig`编码读取，处理可能的BOM头
   - 增加了规则格式验证
   - 完善了错误日志

2. 优化了`replace_text`函数：
   - 按规则字符串长度排序，先替换长字符串避免冲突
   - 增加了替换计数和日志输出
   - 确保替换前后的字符串都是字符串类型

3. 增强了`process_txt_content`函数的编码处理：
   - 增加了双重编码尝试（utf-8 -> gbk）
   - 记录替换前后的文本长度变化，便于验证

4. 在主程序中增加了替换规则加载状态的提示

这些修改确保了替换规则能够正确加载并应用到文本内容中，同时通过日志可以追踪替换的执行情况。如果仍然有问题，可以查看生成的日志文件，里面会记录替换规则的加载情况和具体的替换次数。
