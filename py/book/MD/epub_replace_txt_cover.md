...
# 以下是一个可视化的EPUB内容替换工具，支持封面替换和文本替换（基于JSON配置和数据库查询结果）：
...
2. **替换设置**：
    - 支持启用/禁用JSON配置替换
    - 支持启用/禁用数据库替换（数据库规则优先级更高）
    - 可预览替换规则（前20条）
    - 提供数据库连接测试功能

3. **数据来源**：
    - JSON配置：读取脚本同级目录`json/novelMapping.json`
    - 数据库：读取上一级目录`json/DB_CONFIG.json`配置，查询`v_pornographic_novel_replacement_string`视图

4. **操作流程**：
    - 选择EPUB文件和新封面（可选）
    - 加载替换规则（可手动加载或自动加载）
    - 执行替换（会在原文件同目录生成`xxx_modified.epub`）
    - 所有操作记录会显示在日志面板

### 使用前准备：

1. 安装依赖：`pip install pymysql pillow`
2. 确保配置文件存在：
    - `json/novelMapping.json`（脚本同级目录）
    - `../json/DB_CONFIG.json`（上一级目录的json文件夹）
3. 数据库中存在视图`v_pornographic_novel_replacement_string`，包含`old`和`new`字段

程序会自动保存配置到`json/epub_replacer_config.json`，下次启动时自动加载。
 
# 这个错误的原因是**初始化顺序错误**：在创建`log_text`控件之前就调用了`log()`方法，导致访问`self.log_text`时该属性还未定义。

需要修改`__init__`方法中的执行顺序，确保先创建界面控件（包括`log_text`），再执行可能产生日志的操作。
...
def __init__(self, root):
    ...
    self.load_config()  # 问题所在：此时还未创建log_text控件
    ...
    # 创建界面
    self.create_widgets()  # 这里才创建log_text控件
...
**修改说明**：
- `log_text`控件是在`create_widgets()`方法中创建的
- 原代码中`load_config()`在`create_widgets()`之前执行，导致`log()`方法调用时`log_text`还未初始化
- 调整顺序后，先创建所有界面控件，再加载配置和数据，就不会出现`AttributeError`了

这样修改后，第二次执行时就能够正常加载配置并使用日志功能了。
