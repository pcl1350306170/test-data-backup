...
# ==============================
# PPT 转 PDF 核心逻辑（使用 win32com）
# ==============================

def ppt_to_pdf(ppt_path: Path, pdf_path: Path):
    """将单个 PPT/PPS 文件转换为 PDF"""
    try:
        import win32com.client
        powerpoint = win32com.client.Dispatch("PowerPoint.Application")
        # 隐藏警告信息
        powerpoint.DisplayAlerts = False
        powerpoint.Visible = True  # 必须可见，否则可能失败
        ...
        deck.ExportAsFixedFormat(
            ...
            ExportFormat=2,  # ppFixedFormatTypePDF
...
def worker(task_queue, result_callback):
    while not stop_event.is_set():
        try:
            ...
            # 检查是否暂停
            while pause_event.is_set() is False and not stop_event.is_set():
                time.sleep(0.1)

            if stop_event.is_set():
                break

            success, error = ppt_to_pdf(ppt_file, pdf_file)
            result_callback(ppt_file, pdf_file, success, error)
...
class PptToPdfGUI:
    ...
    def monitor_queue(self):
        while not stop_event.is_set():
            if self.task_queue.empty() and all(not t.is_alive() for t in self.workers):
                if self.completed_count == self.total_count:
                    self.root.after(0, self.on_all_done)
                break
            time.sleep(0.5)

    def on_task_complete(self, ppt_file, pdf_file, success, error):
        self.completed_count += 1
        if success:
            msg = f"✅ 成功: {ppt_file.name} → {pdf_file.name}"
        else:
            msg = f"❌ 失败: {ppt_file.name} | {error}"
    ...
    def on_all_done(self):
        ...
        messagebox.showinfo("完成", f"转换完成！共处理 {self.total_count} 个文件。")
